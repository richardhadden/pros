...
// Incoming inline...
  CALL {
    ...
    WHERE main.uid <> passthrough_node.uid AND NOT incoming_relation:MERGED AND incoming_relation.inline
    CALL {
    ...
    RETURN apoc.map.groupByMulti(COLLECT(ptn{ .uid, .real_type, .label, .is_deleted, .rel_type, incoming: incoming_node }), "rel_type") AS incoming_inline
...
// Incoming inline...
    CALL {
      ...
      WHERE mn.uid <> passthrough_node.uid AND NOT incoming_relation:MERGED AND incoming_relation.inline
      CALL {
        WITH passthrough_node
        MATCH (passthrough_node)<-[in_r]-(x)
        WHERE NOT in_r:MERGED
...
WITH main_node, merged_nodes

RETURN main_node, COLLECT(merged_nodes)
