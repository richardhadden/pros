MATCH (main { uid:"fd2ee917887e4f15aa9a8071d743f4e2" })
...
// Inlines...
    CALL {
      ...
      CALL {
        ...
        WITH apoc.map.setValues(related_node, ["rel_type", toLower(type(related_rel)), "deleted_and_has_dependent_nodes", has_inbound_related_node AND related_node.is_deleted]) AS mod_related_node, related_rel
        RETURN related_rel, apoc.map.groupByMulti(COLLECT(mod_related_node), "rel_type") AS grouped_mod_related_node
      }
    ...
    CALL apoc. WHEN (mn IS NOT null ,
    'WITH apoc.map.setValues(mn, [toLower(type(inline_relation)), inlines, "deleted_and_has_dependent_nodes", has_inbound_main AND mn.is_deleted]) AS main_node, inlines, outgoing_relations, incoming_inline, incoming_relations RETURN apoc.map.mergeList(COLLECT(apoc.map.mergeList([main_node, outgoing_relations, incoming_relations, incoming_inline]))) AS value',
    'RETURN null as value', { mn: mn, inline_relation: inline_relation, inlines:inlines, has_inbound_main:has_inbound_main, outgoing_relations: outgoing_relations, incoming_inline:incoming_inline, incoming_relations: incoming_relations }
    
    )
    YIELD value
    RETURN value.value AS merged_main_node
  }
